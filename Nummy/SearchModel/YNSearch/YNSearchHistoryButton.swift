...
open class YNSearchHistoryButton: UIButton {
    ...
    open var searchHistoryImageview: UIImageView!
    ...
    open func initView() {
        ...
        self.searchHistoryImageview = UIImageView(frame: CGRect(x: 8, y: 8, width: self.frame.height-16, height: self.frame.height-16))
        self.searchHistoryImageview.contentMode = .scaleAspectFit
        let search_history = UIImage(named: "burger", in: Bundle(for: YNSearch.self), compatibleWith: nil)
        searchHistoryImageview.image = search_history
        self.addSubview(searchHistoryImageview)
        
        self.textLabel = UILabel(frame: CGRect(x: 16+self.frame.height, y: 0, width: self.frame.width - 40, height: self.frame.height))
